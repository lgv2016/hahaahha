...
static void PID_Reset(void)
{
    ...
    PID_SetParam(&g_infc.pid[YAW_SPEED],5.0,0.1,0,2000,0);
}
void Infan_Control_Init(void)
{
   //g_angle_target.shoot=0;
  // SET_Angle_Target(g_angle_target);
    //g_speed_target.lf=5000;
     g_speed_target.yaw=400;
...
void Speed_In_Control(object_t measure,float deltaT)
{
    ...
    speed_contorl_out.yaw           = ConstrainFloat(speed_contorl_out.yaw,-5000,5000);
    
    speed_contorl_out.pitch         = ConstrainFloat(speed_contorl_out.pitch,-5000,5000);
...
void Angle_Out_Control(object_t measure,float deltaT)
{
    ...
    angle_contorl_out.yaw           = ConstrainFloat(angle_contorl_out.yaw,-5000,5000);
